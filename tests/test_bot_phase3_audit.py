...
from bot.notify.formatting import build_ack_keyboard, parse_callback_data
from bot.notify.telegram import FakeTelegramTransport, TelegramDispatcher
...
class TestValidatorExpiredAcks:
    ...
    def test_daily_acks_expired_but_signal_specific_fresh(self) -> None:
        ...
        try:
            store = GateStateStore(db)
            v = Validator(store)
            from bot.gates.state import SIGNAL_GATE_WINDOW_MINUTES, SIGNAL_SPECIFIC_GATE_NAMES, GATE_ACK_WINDOW_MINUTES
            from datetime import datetime, timezone
...
class TestValidatorRobustness:
    def test_nan_level_does_not_crash_chart_gates(self) -> None:
        from bot.gates.validator import evaluate_chart_gates
        p = AlertPayload.model_construct(
            prefix="SMC", version="v1", event="bos", symbol="EURUSD",
