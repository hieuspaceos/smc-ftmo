"""Per-pair, 3 exit modes: ladder | scale_in | scale_in_middle.

With PnL $ and exit-reason breakdown.
"""
...
def _build_cfg(mode):
    ...
    if mode in ("scale_in", "scale_in_middle"):
...
def main():
    ...
    out_dir = ROOT / "output" / "multipair-3modes"
    out_dir.mkdir(parents=True, exist_ok=True)
    out_md = out_dir / "report.md"
    md_lines = ["# Multi-pair Ladder / Scale_in / Scale_in_Middle — Detail",
                "",
                "Date range: 2016-01-01 → 2026-08-21 (M15).",
                "Account: $100k, 0.55% risk/trade, max 3 trades/day.",
                ""]

    for mode in ["ladder", "scale_in", "scale_in_middle"]:
        md_lines.append(f"## Mode: `{mode}`")
        md_lines.append("")
        md_lines.append("| Pair | N | W | L | WR% | Total R | Total PnL $ | Win $ | Loss $ | Avg W | Avg L | PF | DD% | ROI% | Final $ | Year span |")
        md_lines.append("|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|")
        for pair in pairs:
            cfg = _build_cfg(mode)
            cfg["pairs"] = [pair]
