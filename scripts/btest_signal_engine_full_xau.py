...
SL floor: min_sl_pips=400 for XAUUSD (= $4/oz = $400/lot floor; equivalent to ~1 ATR on M15) - enforced at entry gate.

Execution costs (per config.yaml execution block, FTMO defaults):
  - Spread XAUUSD: 2.0 pips ($2/lot applied at entry). Note: spread cost is large relative to FTMO SL floor ($400/lot).
...
PAIR = "XAUUSD"
# XAUUSD pip convention: 1 pip = $0.01/oz → $1/lot (1 lot = 100 oz).
PIP_SIZE = 0.01
MIN_SL_PIPS_EURUSD = 400.0
SPREAD_PIPS_EURUSD = 2.0
...
def run(window=None, **kwargs):
    ...
    for i in range(start_bar, len(df)):
        ...
        if sl_atr < min_sl_atr or sl_atr > max_sl_atr:
            continue
        # ABSOLUTE PIP FLOOR - XAUUSD live requires >= 400 pips (= $4/oz = $400/lot floor).
        ...
        # For XAUUSD 1 pip = $1 per standard lot (0.01 price * 100 oz). Use pip value $1.
        pip_value_usd = 1.0
...
def _parse_args():
    p = argparse.ArgumentParser(
        description="XAUUSD backtest via SignalEngine + ScaleInExit (with execution costs)",
    )
...
def main() -> int:
    ...
    out_dir = ROOT / "output" / f"backtest_signal_engine_full_xau{suffix}"
    ...
    print(f"min_sl_pips: {MIN_SL_PIPS_EURUSD} (XAUUSD $4/oz / $400/lot floor)")
    if overrides:
    ...
    print(f"Entries rejected by SL 400-pip floor ($4/oz / $400/lot): {sl_rejects}")
    ...
    if trades:
        ...
        print(f"  trades with sl_pips < {MIN_SL_PIPS_EURUSD} ($4/oz / $400/lot): {len(below)} (must be 0)")
